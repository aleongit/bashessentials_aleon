---
layout: post
title:  "Un administrador d’una màquina Linux ha de revisar dia a dia si hi ha connexió cap a totes les màquines contingudes al fitxer /etc/hosts.
Construïu un script que comprovi la connectivitat amb totes les màquines contingudes al fitxer /etc/hosts i mostri el nom de les màquines indicant (infla: està viva / rebost: NO està viva /collbaix: error en ping)
Recordeu que el fitxer hosts pot tenir comentaris. A part, redirigiu la sortida i error del ping per no confondre la sortida del script"
categories: bash
author: aleon
date: 2023-07-18 16:10:00 +0200
---
```bash
#!/bin/bash
#Aleix Leon
...
#0 - infla: està viva.
#1 - rebost: NO està viva.
#2 - collbaix: error en ping.
